...
function initlistdata() {
    $.ajax({
        url : "/yfClassifyInfos/treetable/"+classifyid+"/"+type,
        type : "get",
        async: false,
        success : function (data) {
          //  console.log(JSON.stringify(data));
            var a = '<div class="fenleiTit">\n' +
                '                        \t<a class="fenleiTitLink" onclick="changeData(\''+data.id+'\',\''+data.type+'\')">\n' +
            ...
            for (var i in result) {
                ...
                if (d.type == 2) {
                    ...
                    td = '<td class="leftLi1"><i><a href="javascript:changeData(\''+d.id+'\',\''+d.type+'\')">'+d.name+'</a></i></td>';
                }else if(d.type == 3) {
                    tr = '<tr data-tt-id="'+ d.id + '" data-tt-parent-id="' + d.parentId + '">';
                    td = '<td class="leftLi2"><i><a href="javascript:changeData(\''+d.id+'\',\''+d.type+'\')">'+d.name+'</a></i></td>';
                }else if (d.type == 4){
                    tr = '<tr data-tt-id="'+ d.id + '" data-tt-parent-id="' + d.parentId + '">';
                    td = '<td class="leftLi3"><i><a href="javascript:changeData(\''+d.id+'\',\''+d.type+'\')">'+d.name+'</a></i></td>';
                }
                tr +=td ;
                tr += '</tr>';
                $(".leftMenu").append(tr);
...
function initNowPlace() {
    $.ajax({
        url:"/yfClassifyInfos/getNowPlace/"+classifyid+"/"+type,
        type:"get",
        async: false,
        success : function (data) {
            $("#nowPlace").html('');
            $("#nowPlace").html('<li><i class="iconfont icon-weizhi"></i>当前位置：</li>\n' +
                '            <li><a href="#">首页</a></li>\n' +
                '            <li><a href="zlResult.html">承研单位</a></li>');
            for (var i in data) {
