...
jQuery(function () {
	...
	$(document).on("click",".sxFirst a",function(event){
		if($(this).hasClass("active")){
			...
			$(this).addClass("active").siblings().removeClass("active");
			$(this).parents(".sxFirst").next(".sxSecond").show();
			var sxTit = $(this).parents(".sxBotItem").find(".sxBotTit").text(),
			 	sxCon = $(this).text(),
				sxId = $(this).parents(".sxBotItem").index()+1,
				sxTagId = "sxTag" + sxId,
				sxAdd = "<span class='sxTag " + sxTagId + "'><b>" + sxTit + "</b><em>"+ sxCon + "</em><i class='iconfont icon-guanbi'></i></span>";
			//alert(sxTagId);
			//$(".sxTopCon").append(sxAdd);
			if($(".sxTopCon").children().hasClass(sxTagId)){
				$('.'+sxTagId).find("em").text(sxCon);
			}else{
				$(".sxTopCon").append(sxAdd);	
			}	
		}
	})
	$(document).on("click",".sxTag",function(event){
		$(this).remove();	
	})
