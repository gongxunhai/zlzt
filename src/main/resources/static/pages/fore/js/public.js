...
jQuery(function () {
	...
	$(".fenleiList li:lt(3)").addClass("top");
	...
	$(document).on("click",".sxFirst a",function(event){
		var sxTit = $(this).parents(".sxBotItem").children(".sxBotTit").text(),
			sxCon = $(this).text(),
	...
	$(document).on("click",".serBtn",function(event){
		...
		if($(this).hasClass("active")){
			$(this).removeClass("active");
			$(".serBox").animate({height:oneHeight});
		}else{
			$(this).addClass("active");
			$(".serBox").animate({height:allHeight});
			$(this).find("span").html("隐藏部分知识产权服务");
		}
	})
	//关于我们
	$(".honour li").each(function(index){$(this).attr("data-wow-delay",index/10+"s");});	
	var hisBox = new Swiper('.hisBox .swiper-container', {
		prevButton: '.hisPrev',
		nextButton: '.hisNext',
		spaceBetween: 10,
		slidesPerView:5,
	...
	//会员中心	
	$(".userBox1 li").each(function(index){$(this).attr("data-wow-delay",index/10+"s");});
	//发布新项目
	$('input[type="radio"]').click(function(){
		if($(this).hasClass("active")){
			$(this).next("span").find("input[type='text']").removeAttr("disabled");
		}else{
