...
public interface KjProductDao {
    ...
    @Insert("insert into kj_product(buyNum, cMan, cPhone, fId, haveNum, history, name, pData, pDetail, pImage, place, price, pubTime, salePrice, sId, talk, xfArea) values(#{buyNum}, #{cMan}, #{cPhone}, #{fId}, #{haveNum}, #{history}, #{name}, #{pData}, #{pDetail}, #{pImage}, #{place}, #{price}, #{pubTime}, #{salePrice}, #{sId}, #{talk}, #{xfArea})")
